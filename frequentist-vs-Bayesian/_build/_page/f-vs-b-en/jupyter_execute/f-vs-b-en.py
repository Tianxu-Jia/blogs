...
# 
# 
# Incidence of breast cancer $0.1\%$ in the some place. If the female has breast cancer, the test positive is $90\%$ and if the person has no breast cancer, the test nagative is $90\%$. Suppose, these is a lady, whose the first time test is positive. How much posibility she has breast cancer? How about if the second time her test is still positive? How do you make decision if you were the doctor?
# 
# If we have no information, we randomly take one female, who has the probability of $0.001$ to have breast cancer. This is the prior probability. $p(c)=0.001$. The female has cancer and the test is positive, which means $p(+|c) = 0.9$. So $p(-|c) = 0.1$. If a female has no cancer, the $p(-|\bar c) = 0.9$ and $p(-|\bar c) = 0.1$. 
# 
...
# $$
# 
# So, the first test as positive only means that she has 0.9% probability to have cancer. Maybe at this time the doctor can't confirm the lady has cancer.
# 
...
# $$
# 
# Maybe now the doctor stll can't confirm the lady has concer. She will need further test
# 
